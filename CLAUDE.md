...
**2026-08-04（最新）**：项目完整可运行。`pytest -m "unit"` 全绿（159 passed，原 142 + 新增 17），前端/管理端 build 通过。最新完成：
- **上游 LLM 挂起不再永久"思考中"**（2026-08-04）：实测 DeepSeek 偶发 "200 OK 后 response body 永久不发" 且 httpx read timeout 未触发（timeout=120 传入确认，但对该挂起不生效；实验证明 timeout 机制本身正常），`retrieval_check` 的 sufficiency/rewrite LLM 调用（stream=False）永久挂起 → graph 卡在 retrieve_knowledge → SSE 一个事件不发 → 前端永远"AI 思考中"。修复：① `check_and_retrieve` 两个 LLM 调用包 `asyncio.wait_for(60s)`，超时走既有 fallback（sufficient=ok / 保原文 query）继续出题；② 前端 `submitAnswerStream` 加 180s 整体超时，超时 abort 并抛明确错误提示，杜绝永久思考。新增单测 `test_check_llm_hang_times_out_falls_back`（挂起 → wait_for 超时 → 1 轮结束）。验证：`pytest -m "unit"` 159 passed、前端 build 通过。顺带清理 interview 9 脏数据（2 条无评分答案 + 1 条消息）
- **面试流式评分/题目气泡错位修复**（2026-08-04）：`submitAnswerStream` 中 evaluate 的评分（`streamingText`）在 done 事件前不清空，而 ask_question 的 `question_chunk` 在 score 之后到达 → 题目气泡与评分气泡并存，且模板 `streamingQuestion` 区块写在 `streamingText` 上方 → 视觉上题目先冒出来、评分在后，done 后才恢复正确顺序。修复：`Interview.vue` 交换两流式区块顺序（评分在上、题目在下）+ `onQuestionChunk` 开始时清空 `streamingText` 残留
