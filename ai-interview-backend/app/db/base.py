"""数据库引擎与会话工厂 — 延迟创建 + 调度任务独立引擎

- get_engine / get_session_local: 懒加载全局单例（避免 Alembic 迁移时 import 即建连）
- create_scheduler_engine / create_scheduler_session_factory: 调度任务专用（小连接池、独立事件循环）
- close_db_engine: 应用关闭时释放连接池
"""
