"""数据库异常装饰器 — 将 SQLAlchemy IntegrityError 转业务异常

- handle_db_exceptions: 装饰器，捕获 IntegrityError 中的外键冲突并转换为 ForeignKeyViolationError
- extract_referenced_table: 从 MySQL / SQLite 错误文本中正则提取被引用的表名
"""
...
def extract_referenced_table(error_message: str) -> str:
    ...
    兼容 MySQL 格式 `constraint fails (`db`.`table`, ...)` 与
    SQLite 格式 `FOREIGN KEY constraint failed (table, ...)`。
    """
