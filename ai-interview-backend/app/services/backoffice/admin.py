"""管理员 CRUD 服务 — 创建、查询、修改、停用管理员账号

权限分级：super_admin 可管理其他管理员；admin 仅可管理用户/题库等业务数据。
"""
...
class AdminService:
    ...
    Args:
        db: 数据库会话。
        admin_data: 包含 email / first_name / last_name / password / is_active 的 AdminCreate。

    Returns:
        新创建的 AdminResponse。

    Raises:
        APIException: email 已存在。
    """
        # 检查邮箱是否已存在
    ...
    async def get_admin(self, db: AsyncSession, admin_id: int) -> Optional[AdminResponse]:
        """获取管理员详情。
    ...
    async def list_admins(self, db: AsyncSession, offset: int = 0, limit: int = 100, email: str = None, sort_by: str = None, sort_order: str = "desc") -> List[AdminResponse]:
        """获取所有管理员列表（支持 email 过滤、按字段排序）。
    ...
    Args:
        ...
        limit: 返回数量上限。
        email: 可选的邮箱模糊过滤。
        sort_by: 排序字段，支持 email 或默认 created_at。
        sort_order: 排序方向 asc / desc。

    Returns:
        AdminResponse 列表。
    """
        admin_query = self.get_admins_query(db, email, sort_by, sort_order).offset(offset).limit(limit)
        result = await db.execute(admin_query)
        admins = result.scalars().all()

        return [AdminResponse.model_validate(admin) for admin in admins]

    async def get_admins_query(self, db: AsyncSession, email: str = None, sort_by: str = None, sort_order: str = "desc"):
        """获取管理员查询对象，用于分页
    ...
    async def update_admin(self, db: AsyncSession, admin_id: int, admin_data: dict) -> Optional[AdminResponse]:
        """更新管理员信息。

    Args:
        db: 数据库会话。
        admin_id: 管理员主键。
        admin_data: 待更新字段字典，支持 email / password / first_name / last_name / is_active。

    Returns:
        更新后的 AdminResponse；管理员不存在则返回 None。

    Raises:
        APIException: 新邮箱已被其他管理员占用。
    """
        # 先检查管理员是否存在
        admin_query = select(Admin).where(Admin.id == admin_id)
        result = await db.execute(admin_query)
    ...
    async def change_password(self, db: AsyncSession, admin_id: int, current_password: str, new_password: str) -> bool:
        """修改管理员密码（需要验证当前密码）。

    Args:
        db: 数据库会话。
        admin_id: 管理员主键。
        current_password: 当前密码（明文）。
        new_password: 新密码（明文）。

    Returns:
        是否修改成功；管理员不存在则返回 False。

    Raises:
        APIException: 当前密码不正确。
    """
        # 先检查管理员是否存在
        admin_query = select(Admin).where(Admin.id == admin_id)
        result = await db.execute(admin_query)
    ...
    async def reset_password(self, db: AsyncSession, admin_id: int, new_password: str) -> bool:
